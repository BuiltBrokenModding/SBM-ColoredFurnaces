...
import com.builtbroken.sbmcoloredfurnaces.content.inv.InventoryColoredFurnace;
import com.builtbroken.sbmcoloredfurnaces.content.inv.InventoryColoredFurnaceBottom;
...
public class TileEntityColoredFurnace extends TileEntity implements ITickable
{
    //Inventory
    public final ItemStackHandler inventory = new InventoryColoredFurnace();

    //Wrappers
    private final IItemHandler handlerTop = new RangedWrapper(inventory, 0, 1);
    private final IItemHandler handlerBottom = new InventoryColoredFurnaceBottom(inventory);
    ...
    public void update()
    {
        ...
        if (!this.world.isRemote)
        {
            ...
            if (isBurning != this.isBurning())
            {
                ...
                if (state.getProperties().containsKey(BlockColoredFurnace.BURNING))
                {
                    world.setBlockState(getPos(), state.withProperty(BlockColoredFurnace.BURNING, this.isBurning()));
                }
            }
        }
    ...
    public void setFurnaceType(ItemStack stack)
    {
        if(stack != null && stack.hasTagCompound() && stack.getTagCompound().hasKey("FurnaceType"))
            setFurnaceType(stack.getTagCompound().getString("FurnaceType"));
    }
    ...
    public void readFromNBT(NBTTagCompound compound)
    {
        ...
        if(compound.hasKey("FurnaceType"))
            this.furnaceType = compound.getString("FurnaceType");
    }
    ...
    public NBTTagCompound writeToNBT(NBTTagCompound compound)
    {
        ...
        if(furnaceType != null)
            compound.setString("FurnaceType", furnaceType);
        return compound;
    }
